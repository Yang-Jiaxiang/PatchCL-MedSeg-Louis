...
def hard_neg(samples_p, mean_p, cov_p, samples_n, mean_n, cov_n, beta=0.5, sample_size=500):
    """
    INPUT:
    ...
    sample_size = int number of samples to generate in each iteration
    ...
    with torch.no_grad():
        ...
        negs = np.ndarray(0)
        while True:
            sample = np.random.multivariate_normal(mu_n, sig_n, sample_size)
