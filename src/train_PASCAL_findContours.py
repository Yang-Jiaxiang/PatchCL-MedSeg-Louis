...
print("image pixel counts:", pixel_counts)

# # imgs_np = masks[0, 1].numpy()
# unique, counts = np.unique(masks[0, 0], return_counts=True)
# pixel_counts = dict(zip(unique, counts))
# print("Mask 0 pixel counts:", pixel_counts)

# unique, counts = np.unique(masks[0, 1], return_counts=True)
# pixel_counts = dict(zip(unique, counts))
# print("Mask 1 pixel counts:", pixel_counts)
...
for c_epochs in range(200): #200 epochs supervised SSL
    ...
    for imgs in unlabeled_loader:
        ...
        miou, accuracy, dice = calculate_metrics(out, multi_channel_masks, num_classes)
        print(f"mIoU: {miou}, Accuracy: {accuracy}, Dice: {dice}")
