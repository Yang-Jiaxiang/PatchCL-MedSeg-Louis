...
model = torch.load(f"{base_path}/output/{contrastive_batch_size}-{ContrastieWeights}/SSL-Resnet18_Image-224_patchSize-{contrastive_batch_size}_ContrastieWeights-{ContrastieWeights}_s_23.pth")
teacher_model = torch.load(f"{base_path}/output/{contrastive_batch_size}-{ContrastieWeights}/SSL-Resnet18_Image-224_patchSize-{contrastive_batch_size}_ContrastieWeights-{ContrastieWeights}_t_23.pth")
...
# %%
for c_epochs in range(24, 200): #200 epochs supervised SSL
    step=0
    min_loss = math.inf
    epoch_t_loss=0
    print('Epoch ',c_epochs)
