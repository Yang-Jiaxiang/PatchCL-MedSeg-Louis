...
contrastive_batch_size = 28
embedding_size = 128
img_size = 224
batch_size = 16
num_classes = 2
ContrastieWeights = 0 # PatchCL loss weight
...
def get_dynamic_weight(epoch, end_epochs):
    """
    动态计算PatchCL的权重，根据总训练epoch数和当前epoch数调整权重。
    每10个epoch调整一次，前10个epoch从0.1开始。
    
    参数:
    epoch (int): 当前epoch数
    end_epochs (int): 总训练epoch数
    
    返回:
    float: 动态计算的权重
    """
    interval = 10  # 每10个epoch调整一次权重
    start_weight = 0.1  # 初始权重
    max_weight = 1.0  # 最大权重
    num_intervals = end_epochs // interval  # 总共的调整次数
    weight_increment = (max_weight - start_weight) / num_intervals  # 每次调整的增量
...
def train(model, teacher_model, train_loader, val_loader, optimizer, criterion, dev, start_epochs, end_epochs, step_name, num_classes, img_size, contrastive_batch_size, save_loss_path):
...
def main():
    ...
    model, teacher_model = train(model, teacher_model, train_loader, val_loader, optimizer_pretrain, cross_entropy_loss, dev, supervised_start_epoch, supervised_end_epoch, "supervised-Pretraining", num_classes, img_size, contrastive_batch_size, save_loss_path)
    ...
    SSL_start_epoch = 0
    SSL_end_epoch = 100
    model, teacher_model = train(model, teacher_model, combined_loader, val_loader, optimizer_ssl, cross_entropy_loss, dev, SSL_start_epoch, SSL_end_epoch, "SSL-reliable-st1", num_classes, img_size, contrastive_batch_size, save_loss_path)
    ...
    model, teacher_model = train(model, teacher_model, combined_loader, val_loader, optimizer_ssl, cross_entropy_loss, dev, SSL_step2_start_epoch, SSL_step2_end_epoch, "SSL-remaining-st2", num_classes, img_size, contrastive_batch_size, save_loss_path)
